...
async function analyzeDisruption() {
    ...
    statusElement.textContent = "Analyzing disruption with SupplyGuard AI...";
...
function renderResults(data) {
    ...
    renderImpact(impact);
    renderDisruptionDetails(data.extraction || {}, data.resolved || {});
    renderImpactDetails(impact);
    renderRecommendation(recommendations);
    renderResponseOptions(recommendations);
    renderOrders(impact.prioritized_orders || []);
    renderImpactGraph(impact.impact_graph || {});
...
function renderImpact(impact) {
    const metrics = [
        {
            label: "Impact Status",
            value: formatLabel(impact.impact_status || "Unknown")
        },
        {
            label: "Impact Found",
            value: impact.impact_found ? "Yes" : "No"
        },
        {
        ...
        },
        {
            ...
            value: (impact.at_risk_orders || []).length
        },
        {
            label: "Affected Customers",
            value: (impact.affected_customers || []).length
        },
        {
            label: "Priority Orders",
            value: (impact.prioritized_orders || []).length
        }
    ];

    document.getElementById("impactMetrics").innerHTML = metrics
        .map(metric => `
            <div class="metric">
...
function renderDisruptionDetails(extraction, resolved) {
    ...
    const details = [
        {
            label: "Event Type",
            value: formatLabel(extraction.event_type || "Unknown")
        },
        {
            label: "Supplier",
            value: supplier?.supplier_name || extraction.supplier_name || "Not identified"
        },
        {
            label: "Location",
            value: supplier?.location || extraction.location || "Not identified"
        },
        {
            label: "Start Date",
            value: extraction.start_date || "Not identified"
        },
        {
            label: "Duration",
        ...
        {
            label: "Confidence",
            value: extraction.confidence !== undefined
    ...
    const productNames = products.length
        ...
        : (extraction.affected_products || []).join(", ");

    details.push({
        label: "Affected Products",
        value: productNames || "Not identified"
    });

    document.getElementById("disruptionDetails").innerHTML = details
        .map(item => `
...
function renderImpactDetails(impact) {
    const shipments = impact.affected_shipments || [];
    const inventory = impact.inventory_summary || [];
    const orders = impact.at_risk_orders || [];
    const customers = impact.affected_customers || [];
    ...
    document.getElementById("impactDetails").innerHTML = cards
        .map(card => `
            <div class="metric">
                <div class="metric-label">
...
function renderRecommendation(recommendations) {
    ...
    if (!recommendations.length) {
        container.innerHTML = `
            <div class="recommendation no-action">
                <strong>No action recommended</strong>
                <p>
                    No affected customer order requires an operational response.
                </p>
            </div>
        `;
        return;
    ...
    const action = recommendation.recommended_action || "NO_ACTION";
    ...
    container.innerHTML = `
        <div class="recommendation">
            ...
            <h3>
                Recommended Action: ${escapeHtml(action)}
            </h3>

            <p>
                ${escapeHtml(recommendation.recommendation_reason || "")}
            </p>

            <div class="recommendation-summary">
                <div>
                    <span>Order</span>
                    <strong>${escapeHtml(recommendation.order_id)}</strong>
                </div>

                <div>
                    <span>Shortage</span>
                    <strong>${escapeHtml(String(recommendation.shortage_quantity))} units</strong>
                </div>

                <div>
                    <span>Priority</span>
                    <strong>${escapeHtml(recommendation.priority)}</strong>
                </div>

                <div>
                    <span>Evidence</span>
                    <strong>
                        ${recommendation.evidence_required ? "Required" : "Not required"}
                    </strong>
                </div>
            </div>
        </div>
...
function renderResponseOptions(recommendations) {
    const container = document.getElementById("responseOptions");

    if (!recommendations.length) {
        container.innerHTML = `
            <p class="empty-state">
                No response options are required because no customer order is currently at risk.
            </p>
        `;
        return;
    }
    ...
    container.innerHTML = options
        .map(option => `
            <div class="option-card ${option.feasible ? "feasible" : "not-feasible"}">

                <div class="option-header">
                    <div>
                        <span class="option-action">
                            ${escapeHtml(option.action)}
                        </span>

                        <span class="policy-rule">
                            ${escapeHtml(option.policy_rule || "Policy")}
                        </span>
                    </div>

                    <span class="feasibility">
                        ${option.feasible ? "FEASIBLE" : "NOT FEASIBLE"}
                    </span>
                </div>

                <p>
                    ${escapeHtml(option.reason || "")}
                </p>

                ${
                    option.coverage_quantity !== undefined
                        ? `
                            <div class="option-metrics">
                                <span>
                                    Coverage:
                                    <strong>
                                        ${escapeHtml(String(option.coverage_quantity))} units
                                    </strong>
                                </span>

                                <span>
                                    ${escapeHtml(String(option.coverage_percentage || 0))}% of shortage
                                </span>
                            </div>
                        `
                        : ""
                ...
                <div class="trade-off">
                    ...
                    ${escapeHtml(option.trade_off || "No trade-off specified.")}
                </div>
        ...
        .map(order => `
            <div class="order-card">
                ...
                <div class="order-main">
                    ...
                    <div class="priority-badge ${getPriorityClass(order.priority)}">
                ...
                <div class="order-metrics">
                    ...
                    <div>
                        ...
                        <strong>
                            ${escapeHtml(String(order.fulfillable_quantity))}
                        </strong>
                    </div>
                    ...
                    <div>
                        ...
                        <strong class="shortage">
                            ${escapeHtml(String(order.shortage_quantity))}
                        </strong>
                    </div>
                    ...
                    <div>
                        ...
                        <strong>
                            ${escapeHtml(String(order.priority_score))}
                        </strong>
                    </div>
                    ...
                    <div>
                        ...
                        <strong>
                            ${escapeHtml(order.required_date)}
                        </strong>
                    </div>
    ...
    const typeLabels = Object.entries(typeCounts)
        .map(([type, count]) => `
            <div class="graph-stat">
                <span>${escapeHtml(formatLabel(type))}</span>
    ...
    container.innerHTML = `
        <div class="graph-summary">
            <div class="graph-stat">
                ...
                <strong>${escapeHtml(String(graph.node_count || nodes.length))}</strong>
            </div>

            <div class="graph-stat">
                <span>Total Relationships</span>
                <strong>${escapeHtml(String(graph.edge_count || edges.length))}</strong>
            </div>
...
function renderEvidence(recommendations) {
    ...
    recommendations.forEach(recommendation => {
        (recommendation.evidence || []).forEach(item => {
            const key = `${item.source}|${item.record_id}|${item.fact}`;
        ...
        (recommendation.options || []).forEach(option => {
            (option.evidence || []).forEach(item => {
                const fact = item.fact || item.detail || "";

                const key = `${item.source}|${item.record_id}|${fact}`;
